...
async function run() {
  ...
  // ── Create ZIP Archive ──
  console.log(`\n📦 Creating ZIP archive: venom-ai-${target}.zip...`);
  try {
    const zipData = {};
...
/**
 * Creates a source code ZIP for submission.
 */
async function generateSourceZip() {
  console.log("\n📦 Creating SOURCE CODE archive for submission...");
  try {
    const zipData = {};
    ...
    const zipped = zipSync(zipData);
    writeFileSync(resolve(__dirname, "venom-ai-source.zip"), zipped);
    console.log("✅ Source code ZIP created successfully: venom-ai-source.zip\n");
    console.log("Submit this file as requested in the 'Source Code' section.");
  } catch (e) {
